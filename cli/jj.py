...
def paste(file, title, language, private, api_url):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            click.echo(f"Paste created successfully!")
...
def login(username, password):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            click.echo(f"Login failed: {response.json().get('error', 'Unknown error')}", err=True)
            sys.exit(1)
...
def view(paste_id):
    ...
    try:
        response = requests.get(
            f"{config.get_api_url()}/pastes/{paste_id}",
            headers=get_headers()
        )
