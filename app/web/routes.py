...
def index():
    ...
    for paste in pastes.items:
        paste.highlighted_preview = highlight_code_preview(paste.content, paste.language)
...
def view_paste(unique_id):
    ...
    return render_template('view_paste.html', paste=paste, highlighted_content=highlighted_content)
...
def edit_paste(unique_id):
    ...
    return render_template('edit_paste.html', title='Edit Paste', form=form, paste=paste)
...
def language_filter(language):
    ...
    pastes = Paste.query.filter_by(is_public=True, language=language).order_by(
        Paste.created_at.desc()
    ).paginate(
        page=page, per_page=per_page, error_out=False
    )
    ...
    return render_template('language_filter.html', pastes=pastes, language=language, title=f'{language.title()} Pastes') 